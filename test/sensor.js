...
exports["Sensor - Resolution"] = {
  ...
  ioPluginProvidesBitResolution(test) {
    test.expect(1);
...
exports["Sensor - Analog"] = {
  ...
  instanceof(test) {
    test.expect(1);
    test.equal(new Sensor({}) instanceof Sensor, true);
    test.done();
  },

  shape(test) {
    let propsActual = Object.getOwnPropertyNames(this.sensor);
    let propsExpected = Object.getOwnPropertyNames(this.members);
    let methodsActual = Object.getOwnPropertyNames(Object.getPrototypeOf(this.sensor));
    ...
    this.methods.forEach(proto => test.ok(methodsActual.includes(proto), `missing '${proto}' sensor prototype method`));
    propsExpected.forEach(property => test.ok(propsActual.includes(property), `missing '${property}' sensor instance property`));
  ...
  threshold(test) {
    ...
    test.strictEqual(spy.callCount, 1, `tick ${tickAccum}: change event handler should be called first time at tick ${this.defShape.freq}`);
    ...
    test.strictEqual(this.sensor.raw, raw, `tick ${tickAccum}: sensor raw property expected to be the last value (${raw}) injected`);
    test.strictEqual(this.sensor.value, raw, `tick ${tickAccum}: sensor value property expected to be the last value (${raw}) injected`);
    test.strictEqual(spy.callCount, 1, `tick ${tickAccum}: change event handler should not be called at tick ${this.defShape.freq * 2}; new median within threshold`);
    ...
    test.strictEqual(this.sensor.raw, raw, `tick ${tickAccum}: sensor raw property expected to be the last value (${raw}) injected`);
    test.strictEqual(this.sensor.value, raw, `tick ${tickAccum}: sensor value property expected to be the last value (${raw}) injected`);
    test.strictEqual(spy.callCount, 2, `tick ${tickAccum}: change event handler should be called at tick ${this.defShape.freq * 3}; new median on threshold boundary`);
    ...
    test.strictEqual(this.sensor.raw, raw, `tick ${tickAccum}: sensor raw property expected to be the last value (${raw}) injected`);
    test.strictEqual(this.sensor.value, raw, `tick ${tickAccum}: sensor value property expected to be the last value (${raw}) injected`);
    test.strictEqual(spy.callCount, 2, `tick ${tickAccum}: change event handler should not be called at tick ${this.defShape.freq * 4}`);
    ...
    test.strictEqual(this.sensor.raw, raw, `tick ${tickAccum}: sensor raw property expected to be the last value (${raw}) injected`);
    test.strictEqual(this.sensor.value, raw, `tick ${tickAccum}: sensor value property expected to be the last value (${raw}) injected`);
    test.strictEqual(spy.callCount, 2, `tick ${tickAccum}: change event handler should not be called at tick ${this.defShape.freq * 5}; new median same as last`);
    ...
    test.strictEqual(this.sensor.raw, raw, `tick ${tickAccum}: sensor raw property expected to be the last value (${raw}) injected`);
    test.strictEqual(this.sensor.value, raw, `tick ${tickAccum}: sensor value property expected to be the last value (${raw}) injected`);
    test.strictEqual(spy.callCount, 3, `tick ${tickAccum}: change event handler should be called at tick ${this.defShape.freq * 6}; on threshold boundary`);
    ...
    test.strictEqual(this.sensor.raw, raw, `tick ${tickAccum}: sensor raw property expected to be the last value (${raw}) injected`);
    test.strictEqual(this.sensor.value, raw, `tick ${tickAccum}: sensor value property expected to be the last value (${raw}) injected`);
    test.strictEqual(spy.callCount, 3, `tick ${tickAccum}: change event handler should not be called at tick ${this.defShape.freq * 7}; median change less than threshold`);
    ...
    test.strictEqual(this.sensor.raw, raw, `tick ${tickAccum}: sensor raw property expected to be the last value (${raw}) injected`);
    test.strictEqual(this.sensor.value, raw, `tick ${tickAccum}: sensor value property expected to be the last value (${raw}) injected`);
    test.strictEqual(spy.callCount, 3, `tick ${tickAccum}: change event handler should not be called at tick ${this.defShape.freq * 8} since change does not exceed threshold`);
    ...
    test.strictEqual(this.sensor.raw, raw, `tick ${tickAccum}: sensor raw property expected to be the last value (${raw}) injected`);
    test.strictEqual(this.sensor.value, raw, `tick ${tickAccum}: sensor value property expected to be the last value (${raw}) injected`);
    test.strictEqual(spy.callCount, 4, `tick ${tickAccum}: change event handler should not be called at tick ${this.defShape.freq * 10} since change does not exceed threshold`);
    ...
    tickAccum += tickDelta;
    test.strictEqual(spy.callCount, 5, `tick ${tickAccum}: change event handler should be called at tick ${this.defShape.freq * 11}; changed threshold moves existing filtered value outside range`);
  ...
  limit(test) {
    const callback = this.analogRead.args[0][1];
    const dataSpy = this.sandbox.spy();
    const limitSpy = this.sandbox.spy();
    const lowerSpy = this.sandbox.spy();
    const upperSpy = this.sandbox.spy();
    let newShape;
    let raw;
    let filtered;
    let tickDelta;
    ...
    // elapsed time is now at the end of the second event throttling interval
    tickAccum += tickDelta;
    test.strictEqual(dataSpy.callCount, 2, `tick ${tickAccum}: data event handler should be called again time at tick ${this.defShape.freq * 2}`);
    ...
    test.strictEqual(limitSpy.callCount, 1, `tick ${tickAccum}: limit event handler should be called at ${this.defShape.freq} ticks; value at lower limit`);
    test.strictEqual(lowerSpy.callCount, 1, `tick ${tickAccum}: limit:lower event handler should be called at ${this.defShape.freq} ticks; value at lower limit`);
    test.strictEqual(upperSpy.callCount, 0, `tick ${tickAccum}: limit:upper event handler should not be called at ${this.defShape.freq} ticks; value at lower limit`);
    ...
    // elapsed time is now at the end of the sixth event throttling interval
    tickAccum += tickDelta;
    test.strictEqual(dataSpy.callCount, 6, `tick ${tickAccum}: data event handler should be called every multiple of ${this.defShape.freq} ticks`);
    test.strictEqual(limitSpy.callCount, 2, `tick ${tickAccum}: limit event handler should be called at ${this.defShape.freq} ticks; value at upper limit`);
    test.strictEqual(lowerSpy.callCount, 1, `tick ${tickAccum}: limit:lower event handler should not be called at ${this.defShape.freq} ticks; value at upper limit`);
    test.strictEqual(upperSpy.callCount, 1, `tick ${tickAccum}: limit:upper event handler should be called at ${this.defShape.freq} ticks; value at upper limit`);
    ...
    // elapsed time is now at the end of the seventh event throttling interval
    tickAccum += tickDelta;
    test.strictEqual(dataSpy.callCount, 7, `tick ${tickAccum}: data event handler should be called every multiple of ${this.defShape.freq} ticks`);
    test.strictEqual(limitSpy.callCount, 3, `tick ${tickAccum}: limit event handler should be called at ${this.defShape.freq} ticks; value at lower limit`);
    test.strictEqual(lowerSpy.callCount, 2, `tick ${tickAccum}: limit:lower event handler should be called at ${this.defShape.freq} ticks; value at lower limit`);
    test.strictEqual(upperSpy.callCount, 1, `tick ${tickAccum}: limit:upper event handler should not be called at ${this.defShape.freq} ticks; value at lower limit`);
    ...
    // elapsed time is now at the end of the eighth event throttling interval
    tickAccum += tickDelta;
    test.strictEqual(dataSpy.callCount, 8, `tick ${tickAccum}: data event handler should be called every multiple of ${this.defShape.freq} ticks`);
    test.strictEqual(limitSpy.callCount, 4, `tick ${tickAccum}: limit event handler should be called at ${this.defShape.freq} ticks; value at upper limit`);
    test.strictEqual(lowerSpy.callCount, 2, `tick ${tickAccum}: limit:lower event handler should not be called at ${this.defShape.freq} ticks; value at upper limit`);
    test.strictEqual(upperSpy.callCount, 2, `tick ${tickAccum}: limit:upper event handler should be called at ${this.defShape.freq} ticks; value at upper limit`);
  ...
  freq(test) {
    ...
    tickAccum += tickDelta;
    test.strictEqual(spy.callCount, 2, `tick ${tickAccum}: data event handler should be called second time at tick ${this.defShape.freq + newFreq}`);
...
exports["Sensor - Digital"] = {
  ...
  data(test) {
    const data = this.digitalRead.args[0][1];
    const spy = this.sandbox.spy();
