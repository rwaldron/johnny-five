...
var Controllers = {
  PCA9685: {
    ...
    ledWrite: {
      value: function(pin, state) {

        var on, off;

        on = 0;
        off = this.pwmRange[1] * state.value / 255;
  ...
  Standard: {
    ...
    ledWrite: {
      value: function(pin, state) {
        ...
        if (state.value !== this.io.LOW && state.value !== this.io.HIGH && !this.board.pins.isPwm(this.pin)) {
          Board.Pins.Error({
            pin: this.pin,
            type: "PWM",
            via: "Led",
        ...
        if (state.mode === this.io.MODES.OUTPUT) {
          this.io.digitalWrite(this.pin, state.value);
        }
