...
function Accelerometer( opts ) {
  ...
  loPass = function(alpha) {
    ...
    axes.forEach(function(axis) {
      ...
      self.smooth[axis] = (last[axis] * alpha) + (1 - alpha) * self.raw[axis];
    });
    return self.smooth;
  };
  ...
  setInterval(function() {
    ...
    this.smooth = this.smoother();

    // I'm sure LS will be useful at some point but we'll keep it
    // conformable with iranges
    changed = [
      last.x - this.smooth.x,
      last.y - this.smooth.y,
      last.z - this.smooth.z
    ];

    // this is ugly. Will replace when I can replace the change event.
    last.x = this.smooth.x;
    last.y = this.smooth.y;
    last.z = this.smooth.z;
