...
function Accelerometer( opts ) {
  ...
  loPass = function( alpha ) {
    ...
    axes.forEach( function( axis, index ) {
      ...
      self.smooth[axis] = (last[axis] * alpha) + (1 - alpha) * self.accel[axis];
      // update change for the axischange event
      changed[index] = last[axis] - self.smooth[axis];
      // might as well set the last value here
      last[axis] = self.smooth[axis];
    });
    return self.smooth;
  };
