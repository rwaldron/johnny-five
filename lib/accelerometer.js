  ...
  MMA8452: {
    ...
    initialize: {
      value: function(opts, dataHandler) {
        ...
        state.sensitivity = opts.sensitivity || 2048;
        state.scale = 2;
        ...
        // Standby mode
        this.io.i2cWrite(address, this.REGISTER.CONTROL1, state.ctrl);
        ...
        state.cfg &= 0xFC;
        state.cfg |= state.scale >> 2;

        this.io.i2cWrite(address, this.REGISTER.CONFIG, state.cfg);

        state.ctrl &= 0xC7;
        state.ctrl |= state.odr << 3;
        ...
        this.io.i2cRead(address, this.REGISTER.READREGISTER, READLENGTH, function(data) {
          dataHandler.call(this, {
            // MMA8452 produces left align values from its registers, so
            // right align to 12 bit values
            x: int16(data[0], data[1]) >> 4,
            y: int16(data[2], data[3]) >> 4,
            z: int16(data[4], data[5]) >> 4,
          });
        }.bind(this));
      },
    },
