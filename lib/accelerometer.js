...
function Accelerometer( opts ) {
  ...
  this.data = (function( types, axes ){
    var container = {};
    // Build out arrays for combinations of data type and axis
  ...
  this.smoother = (opts.smoother || function() {
    ...
    this.axes.forEach(function( axis ) {
      var priorsm, accel, currsm;
      // Exponential smoothing
      // see http://people.duke.edu/~rnau/411avg.htm
      accel = this.data.accel[ axis ][ 0 ];
      priorsm = this.data.smooth[ axis ][ 0 ];
      // bootstrap for initial state
      if ( this.initial ) {
        this.initial = false;
        this.update("smooth", axis, accel);
      } else {
        // convolves past smoothed/nonsmooth readings
        currsm = priorsm + alpha * ( accel - priorsm );
        // update current smooth 
        this.update("smooth", axis, currsm);
      }
    }, this);
  ...
  this.pins.forEach(function( pin, index ) {
    ...
    this.firmata.analogRead( pin, function( data ) {
      ...
      var accel =  acceleration( data, this.voltage, this.sensitivity );
      // The output we're interested in most of the time
      this.update( "accel", paxis, accel );
