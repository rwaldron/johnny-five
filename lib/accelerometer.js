...
const Controllers = {
  ...
  MPU6050: {
    initialize: {
      value(options, callback) {
        ...
        driver.on("data", ({accelerometer}) => callback(accelerometer));
      }
    },
    toGravity: {
      value(raw) {
  ...
  ADXL335: {
    ...
    toGravity: {
      ...
      value(value, axis) {
        const read = analogToGravity.call(this, value, axis);
  ...
  LSM303C: {
    initialize: {
      value(options, callback) {
        const IMU = require("./sip");
        const driver = IMU.Drivers.get(this.board, "LSM303C", options);
        driver.on("data", ({accelerometer}) => callback(accelerometer));
      }
    },
    toGravity: {
