...
function Accelerometer( opts ) {
  ...
  this.smoother = (opts.smoother || function() {
    ...
    this.axes.forEach(function( axis ) {
      ...
      accel = this.data.accel[ axis ];
      smooth = this.data.smooth[ axis ];
      // bootstrap for initial state
      if ( this.initial ) {
        this.initial = false;
        this.data.smooth[ axis ] = update( smooth, accel[ 0 ], this.extent );
      } else {
        // convolves past smoothed/nonsmooth readings
        currsm = smooth[ 0 ] + alpha * ( accel[ 0 ] - smooth[ 0 ] );
        // update current smooth
        this.data.smooth[ axis ] = update( smooth , currsm, this.extent );
      }
    }, this);
