...
var Controllers = {
  ...
  MMA7361: {
    ...
    initialize: {
      value: function(opts, dataHandler) {
        var sleepPin = opts.sleepPin;

        if (sleepPin !== undefined) {
          this.board.pinMode(sleepPin, 1);
          this.board.digitalWrite(sleepPin, 1);
        }
  ...
  if (typeof this.initialize === "function") {
    this.initialize(opts, function(data) {
      ...
      Object.keys(data).forEach(function(axis) {
        ...
        if (opts.autoCalibrate && sensor.calibration < calibrationSize) {
          ...
          console.log("New zeroV[" + axis +"] = " + state.zeroV[axisIndex]);
        }
