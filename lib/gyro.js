...
function Gyro(opts) {
  ...
  this.pins.forEach(function(pin, index) {
    ...
    this.io.analogRead(pin, function(data) {
      ...
      if (!isCalibrated &&
        (state.x.calibration.length === 100 &&
          state.y.calibration.length === 100)) {

        isCalibrated = true;
        state.x.zeroVoltage = (sum(state.x.calibration) / 100) | 0;
        state.y.zeroVoltage = (sum(state.y.calibration) / 100) | 0;

      } else {
        if (sensor.calibration.length < 100) {
