    ...
    initialize: {
      value: function(opts) {
        ...
        this.io.i2cRead(address, this.COMMANDS.READREGISTER, READLENGTH, function(bytes) {
          ...
          state.x = int16(bytes[0], bytes[1]);
          state.y = int16(bytes[4], bytes[5]);
          state.z = int16(bytes[2], bytes[3]);
    ...
    scaled: {
      get: function() {
        ...
        return {
          ...
          z: state.z ? state.z * state.scale : null
        };
      }
    }
  },
    ...
    initialize: {
      value: function() {
        ...
        this.io.i2cRead(address, this.COMMANDS.READREGISTER, READLENGTH, function(bytes) {
          ...
          state.x = (((bytes[0] << 8) + bytes[1]) / 10) | 0;
          state.y = null;
          state.z = null;
...
function Compass(opts) {
  ...
  setInterval(function() {
    ...
    if (state.heading !== heading) {
      ["headingchange", "change"].forEach(function(event) {
        this.emit(event, {
          now: heading,
