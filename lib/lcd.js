...
LCD.prototype.print = function( message, dontProcessSpecials ) {
  var state, hasCharacters;
  ...
  if ( message.length === 1 ) {
    ...
    if ( hasCharacters ) {
      var lcd = this;
      var processed = message.replace(RE_SPECIALS, function(match, name) {
        var address = state.characters[name];
        return 'number' === typeof address ? String.fromCharCode(address) : match;
      });
      this.print(processed, true);
    } else {
      LCD.hilo.call( this, function() {
        var k, chars, char;
