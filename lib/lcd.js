...
LCD.prototype.print = function( message ) {
  var words, state, hasCharacters;

  state = priv.get(this);
  message = message + "";
  hasCharacters = /\:(.*?)\:/g.test(message);
  ...
  if ( message.length === 1 ) {
    ...
    if ( hasCharacters ) {
      words = new Words(message);
      state = priv.get(this);

      words.forEach(function( word, i ) {
        var key, address;

        if ( rspecial.test(word) ) {
          address = state.characters[ word.slice(1, -1) ];
        }
 ...
 * Words
 *
 * Parse a string of input into a set of ordered words, spaces and special characters
 *
 */
function Words(str) {
  Array.call(this);
  ...
  while (word = this.next()) {
    this[++i] = word;
    this.length++;
  }
}

Words.prototype = Object.create(Array.prototype);

Words.prototype.next = function() {
  var current, special, isSpecial, word, chars, char, length;

  current = this.raw.slice( this.seen.length );
  special = current.match( rspecial );
  isSpecial = special && special.length;
  word = "";

  if ( this.seen.length === this.raw.length ) {
    return null;
  }
  ...
  if ( isSpecial ) {
    ...
    if ( rstartingSpace.test(current) ) {
      this.seen += " ";
      return " ";
    } else {
      chars = current.split("");
