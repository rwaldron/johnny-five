...
LCD.prototype.print = function( message ) {
  var parts, state, characters, hasCharacters, character, created;

  state = priv.get(this);
  message = message + "";
  characters = /:.+:/g.exec(message);
  hasCharacters = characters && characters.length;

  // Don't trim _empty_ spaces.
  if ( message.length > 1 ) {
