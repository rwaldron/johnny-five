...
var rchar = /:([a-z0-9\+\-_]+):/g;

LCD.prototype.print = function( message ) {
  var parts, state, characters, isCreating, created;
  ...
  if ( message.length === 1 ) {
    LCD.hilo.call( this, function() {
      ...
      this.command( " ".charCodeAt(0) );
    });
  } else {

    if ( rchar.test(message) ) {
      parts.forEach(function( word, i ) {
        var trailing, matches, key, address;

        trailing = i === parts.length - 1 ? "" : " ";
        matches = rchar.exec( word );
        isNew = false;
...
LCD.prototype.createChar = function( name, charMap ) {
  ...
  if ( typeof name === "number" ) {
    ...
    if ( state.index === LCD.MEMORYLIMIT ) {
      address = state.index = -1;
    }
  }
...
LCD.prototype.useChar = function( name ) {
  ...
  if ( !state.characters[ name ] ) {
    ...
    this.createChar( address, LCD.Characters[ name ] );
