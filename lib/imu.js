            ...
            setTimeout(function() {
              io.i2cReadOnce(address, this.REGISTER.READ_RESULT, numBytes, function(data) {
                ...
                if (isTemperatureCycle) {
                  ...
                  var seapress = compensated / Math.pow(1 - elevation * 0.0000225577, 5.255877);
    ...
    initialize: {
      value: function(board, opts) {
        ...
        var readCycle = function() {
          ...
          //    be read from the peripheral register.
          //
            ...
            io.i2cReadOnce(address, this.REGISTER.READ, 3, function(data) {
              ...
              if (isTemperatureCycle) {
                ...
                */
                ...
                raw.temperature = computed.temperature = value >> 2;
                actual.temperature = (raw.temperature / 32) - 50;
              } else {
                /*
                | DATA MSB                      | DATA LSB                      |
                | 7 | 6 | 5 | 4 | 3 | 2 | 1 | 0 | 7 | 6 | 5 | 4 | 3 | 2 | 1 | 0 |
                ...
                */
                ...
                raw.humidity = computed.humidity = value >> 4;
                actual.humidity = (raw.humidity / 16) - 24;
                linear.humidity = actual.humidity - (Math.pow(actual.humidity, 2) * cof.A2 + actual.humidity * cof.A1 + cof.A0);
                computed.humidity = linear.humidity + (actual.temperature - 30) * (linear.humidity * cof.Q1 + cof.Q0);
              }
...
function IMU(opts) {
  ...
  if (controller == null) {
    throw new Error("Missing IMU controller");
  }
