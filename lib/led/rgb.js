  ...
  Object.defineProperties(this, {
    ...
    update: {
      value: function(colors) {
        ...
        state.values = ToScaledRGB(state.intensity, colors);
...
function ToScaledRGB(intensity, colors) {
  ...
  if ((flags | 0x04) === 0x04) {
    ...
    if (Array.isArray(input)) {
      ...
      if (input.match(/^#?[0-9A-Fa-f]{6}$/)) {
        ...
        // color name
        return ToRGB(converter.keyword.rgb(input.toLowerCase()));
      }
    }
  } else {
...
RGB.prototype.color = function(red, green, blue) {
  ...
  var update = ToRGB(red, green, blue);
...
RGB.prototype[Animation.normalize] = function(keyFrames) {
  ...
  if (keyFrames[0] === null) {
    keyFrames[0] = state.values || { red: 0, green: 0, blue: 0 };
  }
  ...
  return keyFrames.reduce(function(accum, frame) {
    ...
    if (frame !== null) {
      ...
      if (!frame.easing) {
        normalized.easing = "linear";
      }

      normalized.value = ToScaledRGB(intensity, ToRGB(color));
    }
...
RGB.prototype[Animation.render] = function(color) {
