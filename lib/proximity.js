...
var Controllers = {
  GP2Y0A21YK: {
    ...
    toCm: {
      value: function(raw) {
        return +(12343.85 * Math.pow(raw, -1.15)).toFixed(3);
      }
    }
  },
  GP2D120XJ00F: {
  ...
  GP2Y0A02YK0F: {
    ...
    toCm: {
      value: function(raw) {
        return +(10650.08 * Math.pow(raw, -0.935) - 10).toFixed(3);
      }
    }
  },
  GP2Y0A41SK0F: {
...
function Proximity(opts) {
  ...
  var isFiltered = opts.filter;
  var filterDefaults = {
    samples: 5,
    soften: 2,
    deviation: 2
  };
  var filterOpts = typeof opts.filter === "object" ? Object.assign(filterDefaults, opts.filter) : filterDefaults;
  ...
  Object.defineProperties(this, {
    ...
    inches: {
      get: function() {
        return +(this.centimeters * 0.39).toFixed(3);
      }
    },
    in: {
      get: function() {
    ...
    filtered: {
      get: typeof opts.filter === "function" ? opts.filter : function() {
        var average = this.samples.reduce(function(a, b) {
          return a + b;
        }) / this.samples.length;
        var isSoften = typeof filterOpts.soften === "number";

        raw = average;

        return {
          cm: isSoften ? soften(this.cm) : this.cm,
          centimeters: isSoften ? soften(this.centimeters) : this.centimeters,
          in : isSoften ? soften(this.in) : this.in,
          inches: isSoften ? soften(this.inches) : this.inches,
        };

        function soften(num) {
          return Math.round(num * Math.pow(10, filterOpts.soften)) / Math.pow(10, filterOpts.soften);
        }
      }
    }
  });
  ...
  if (typeof this.initialize === "function") {
    ...
    this.initialize(opts, function(data) {
      ...
      if (isFiltered && typeof filterOpts.deviation === "number") {
        data = raw && data > raw * (filterOpts.deviation) ? raw : data;
      }

      if (!isFiltered || this.samples.length === filterOpts.samples) {
        this.samples.shift();
      }
  ...
  setInterval(function() {
    ...
    var data = isFiltered ? this.filtered : {
      cm: this.cm,
    ...
    if (isFiltered && rawFiltered !== lastFiltered) {
      lastFiltered = rawFiltered;
      this.emit("change:filtered", data);
    }
