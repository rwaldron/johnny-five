...
var Controllers = {
  ADAMOTOV1: {
    ...
    initialize: {
      value: function (opts) {
        if (this.pins && this.pins.pwm) {

          _.forIn(this.HBRIDGE, function(value, key) {
            if (value.pwm === this.pins.pwm) {
          ...
          this.motorConfig = this.HBRIDGE[opts.name];
          this.name = opts.name;

          this.opts.pins = {
            pwm: this.motorConfig.pwm
          };
        }

        if (!this.motorConfig) {
          throw new Error("Cannot find motor config");
        }

        this.io.pinMode(this.pins.pwm, this.io.MODES.PWM);

        if (!this.board.ADAMOTOV1) {
          var register = new ShiftRegister({
            pins: this.SHIFT_REGISTER
          });
          ...
          var latchOn = function(bit) { return shield.state |= (1 << bit); };
          var latchOff = function(bit) { return shield.state &= ~(1 << bit); };
          var latch = function(bit, on) { return on ? latchOn(bit) : latchOff(bit); };

          this.board.ADAMOTOV1 = {
            update: function(config, switchAOn, switchBOn) {
              var currentState = shield.state;

              latch(config.switchBits.A, switchAOn);
              latch(config.switchBits.B, switchBOn);

              if(shield.state != startingState) {
                register.send(shield.currentState);
              }
            },
            forward: function(config) {
              self.board.ADAMOTOV1.update(config, true, false);
            },
            reverse: function(config) {
              self.board.ADAMOTOV1.update(config, false, true);
            }
          };
        }
      }
    ...
    dir: {
      value: function(speed, dir) {
        var motorShield = this.board.ADAMOTOV1;

        this.stop();

        if (motorShield[dir.name]) {
          motorShield[dir.name](this.motorConfig);
        }
...
function Motor(opts) {
  ...
  if (typeof this.opts.device === "undefined") {
    ...
    this.opts.device = (typeof this.opts.pins === "undefined" && typeof this.opts.name === "undefined") ?
