 ...
 * @constructor
 *
 * @param {Object} opts Options: pin|pins{pwm, dir[, cdir]}
 * @param {Number} pin: A single pin for basic motor control
 * @param {Number} pins: a combination of pins specified for speed and direction control
 *                      pwm: pulse width modulation pin used to control motor speed
 *                      dir: a pin that is used to specify primary direction, or as a directional "flag" on some shields
 *                      cdir: an optional pin for specifying counter direction as required by some shields - an alternative to specifying this pin in code is to connect it to the dir pin via a NOT gate
 *
 *
 ...
 *
 *
 * Initializing Bi-Directional DC Motor Shields that only require one pin to specify direction:
 *
 *    new five.Motor([ 3, 12]);
 *
 * ...is the same as...
 ...
 *
 *
 * Initializing Bi-Directional DC Motors that require two pins to specify direction:
 * ex. Freetronics H-Bridge Motor Driver Shield (http://www.freetronics.com/products/hbridge-dual-channel-h-bridge-motor-driver-shield)
 *
...
function Motor( opts ) {
  ...
  //specify directional pins, if directional motor type
  if ( this.type === Motor.TYPE.DIRECTIONAL ) {
    this.firmata.pinMode( this.pins.dir, this.firmata.MODES.OUTPUT );
    if (this.pins.cdir) { this.firmata.pinMode( this.pins.cdir, this.firmata.MODES.OUTPUT ); }
  }
  ...
  var method = function( speed ) {
    ...
    if ( this.type === Motor.TYPE.DIRECTIONAL ) {
      ...
      if ( speed > this.threshold ) {
        ...
        if (this.pins.cdir) { this.firmata.digitalWrite( this.pins.cdir, !dir.value ); }
        this.start( speed );
