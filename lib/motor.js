  ...
  ShiftRegister: {
    initialize: {
      value: function (opts) {
        ...
        if (!this.board.registry) {
          this.board.registry = {};
        }

        if (!this.board.registry[rKey]) {
          this.board.registry[rKey] = new ShiftRegister({
            board: this.board,
            pins: opts.register
          });
        }
    ...
    dir: {
      value: function(speed, dir) {
        var rKey = registerKey(this.opts.register),
            register = this.board.registry[rKey];

        this.stop();

        this.updateShiftRegister(register, dir.name);
    ...
    updateShiftRegister: {
      value: function(register, dir) {
        var switchPositions = { a: false, b: false };
        var latchState = register.state;

        switch (dir) {
          case "forward":
            switchPositions = { a: true, b: false };
            break;

          case "reverse":
            switchPositions = { a: false, b: true };
            break;
        }

        latchState = latch(latchState, priv.get(this).bits.a, switchPositions.a);
        latchState = latch(latchState, priv.get(this).bits.b, switchPositions.b);

        if (register.state !== latchState) {
