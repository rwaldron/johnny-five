  ...
  ADAMOTOV1: {
      ...
      value: function (opts) {
        ...
        if (!this.board.ADAMOTOV1) {
          ...
          this.board.ADAMOTOV1 = {
            update: function(switchABit, switchAOn, switchBBit, switchBOn) {
              latch(switchABit, switchAOn);
              latch(switchBBit, switchBOn);
              register.send(shield.state);
            },
            forward: function(switchABit, switchBBit) {
              self.board.ADAMOTOV1.update(switchABit, true, switchBBit, false);
            },
            reverse: function(switchABit, switchBBit) {
              self.board.ADAMOTOV1.update(switchABit, false, switchBBit, true);
            }
          };
        }
      }
    ...
    dir: {
      value: function(speed, dir) {
        var switchBits = this.HBRIDGE[this.pins.pwm];
        var motorShield = this.board.ADAMOTOV1;

        this.stop();

        if (switchBits && motorShield[dir.name]) {
          motorShield[dir.name](switchBits.A, switchBits.B);
        }
