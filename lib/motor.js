  ...
  DIRECTIONAL: {
    ...
    dir: {
      ...
      value: function(speed, dir) {
        ...
        if (speed === this.speed() && this.direction.value === dir.value) {
          return;
        }
...
Motor.prototype.setPin = function(pin, value) {
  ...
  if (state.writes.digital[pin] !== value || !state.isOn || !state.braking) {
    canWrite = true;
  } else {
    if (state.braking) {
      canWrite = true;
  ...
  state.writes.digital[pin] = value;
};
...
Motor.prototype.setPWM = function(pin, value) {
  ...
  if (state.writes.pwm[pin] !== value || !state.isOn) {
    canWrite = true;
  } else {
    if (state.isOn && state.braking) {
      canWrite = true;
    }
  }
