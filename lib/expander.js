  ...
  GROVEPI: {
    ...
    initialize: {
      value: function(opts) {
        ...
        for (var i = 0; i < 16; i++) {
          ...
          if (i <= 1 || (i >= 9 && i < 14)) {
            // There are no pins for:
            // DO, D1, D9, D10, D11, D12, D13
            this.pins.push({
              supportedModes: [],
              mode: 0,
              value: 0,
            ...
            if (i >= 14 && i <= 16) {
              ...
              // ANALOG "read"
              this.pinMode(i, this.MODES.ANALOG);
            } else {
      ...
      value: function(pin, callback) {
        ...
        var read = function() {
          ...
          this.io.i2cReadOnce(this.address, 1, function(data) {
            if (this.pins[pinIndex].value !== data) {
              this.pins[pinIndex].value = data;

              this.emit("digital-read-" + pin, data);
            }
            read();
          }.bind(this));
        }.bind(this);
    ...
    analogRead: {
      value: function(pin, callback) {
        ...
        var read = function() {
          this.io.i2cWrite(
            this.address, [
              this.COMMANDS.ANALOG_READ,
              pinIndex,
              0, 0
            ]
          );

          this.io.i2cReadOnce(this.address, 4, function(data) {
    ...
    pingRead: {
      value: function(pin, callback) {
        ...
        this.on("ping-read-" + pin, callback);
