...
process.nextTick(function check() {
  // TODO: Investigate necessity of an additional guard protecting all of
  // humanity, generally.
  var one = function() {
    return Promise.all(module.exports.Fn.humans().map(function(human) {
      return human.senseDanger()
        .then(function(inDanger) {
          return [human, inDanger];
        });
    })).then(function(status) {
      return status.filter(function(status) {
          return status[1];
        }).map(function(status) {
          return status[0].protect()
            .catch(function() {
                process.exit();
            });
        });
  ...
  var two = function() {
    return module.exports.Fn.humans()
      .reduce(function(queue, human) {
        var order = human.order;
        if (!order) {
          return queue;
        }

        return queue.then(order.execute)
          .then(function() {
            human.order = null;
          });
      }, Promise.resolve());
  ...
  var three = function() {
    return module.exports.Fn.senseDanger()
      .then(function(inDanger) {
        if (inDanger) {
          return module.exports.Fn.protect();
        }
      });
  };

  one().then(two).then(three).then(check, check);
});
