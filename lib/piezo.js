...
Piezo.normalizeBeat = function(beat) {
  ...
  _.forEach(Piezo.Beats, function(beatElement) {
    if (beatElement[0] === beat) {
      return beatElement[1];
    }
  });
...
Piezo.normalizeNote = function(note) {
  if (typeof note === "string") {
    var normalizedNote = {};

    if (note.length === 3) {
      normalizedNote.duration = note[0];
      normalizedNote.notation = note[1];
      normalizedNote.beat = note[2];
...
Piezo.normalizeMelody = function(melody, defaults) {
  ...
  _.forEach(sheetElements, function(element) {
    ...
    var note = Piezo.normalizeNote(element);
    if (note) {
...
Piezo.noteToFrequency = function(note, duration) {
  var frequency = null;
...
Piezo.prototype.clearTimer = function() {
