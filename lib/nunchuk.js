...
DeviceMap = {
  ...
  "RVL-005": {
    ...
    inReadLoopDo: function (that) {
      ...
      [ that.left_joystick, that.right_joystick ].forEach(function( component ) {
        axes.forEach( function( axis ) {
          var delta = "d" + axis;
          if ( typeof component[ delta ] !== "undefined" ) {
            if ( Math.abs( component[ delta ] ) > this.threshold ) {
    ...
    initialize: function (obj) {
      ...
      obj.up = new Nunchuk.Button( "up", obj );
      obj.down = new Nunchuk.Button( "down", obj );
      obj.left = new Nunchuk.Button( "left", obj );
      obj.right = new Nunchuk.Button( "right", obj );
