...
__.extend( priv, {
  ...
  setJoystickX : function( joystick, x ) {
    var state = priv.get(joystick);
    state.x = x;
    priv.set( joystick, state );
  },

  setJoystickY : function( joystick, y ) {
    var state = priv.get(joystick);
    state.y = y;
    priv.set( joystick, state );
  }
  ...
  "RVL-004": {
    ...
    data: function( data ) {
      ...
      if ( data[0] !== 254 && data[1] !== 254 && data[2] !== 254 ) {
        ...
        priv.setJoystickX( this.joystick, decodeByte( data[0] ) << 2 );

        // Byte 0x01 :  Y-axis data of the joystick
        priv.setJoystickY( this.joystick, decodeByte( data[1] ) << 2 );
