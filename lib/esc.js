...
function ESC(opts) {
  ...
  if (this.type === "bidirectional") {
    if (Number.isNaN(+this.neutral)) {
      throw new Error("Bidirectional speed controllers require a neutral point from 0-100 (number)");
    }
...
/**
 * reverse Set reverse speed
 * @param {Number} speed 0-100
 * @return {Object} instance
 */
ESC.prototype.reverse = ESC.prototype.rev = function(speed) {
  if (this.type !== "bidirectional") {
    this.board.error(
      "ESC",
      "ESC.prototype.reverse() is only available for bidirectional speed controllers"
    );
  }
  return this.speed(__.scale(speed, 0, 100, this.neutral, this.range[0]));
};

/**
 * reverse Set forward speed
 * @param {Number} speed 0-100
 * @return {Object} instance
 */
ESC.prototype.forward = ESC.prototype.fwd = function(speed) {
  if (this.type !== "bidirectional") {
    this.board.error(
      "ESC",
      "ESC.prototype.forward() is only available for bidirectional speed controllers"
    );
  }
  return this.speed(__.scale(speed, 0, 100, this.neutral, this.range[1]));
};
