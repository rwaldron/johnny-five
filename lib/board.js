...
const Serial = {
  ...
  detect: callback => {
    ...
    serialport.list((err, result) => {
      ...
      console.log("Available", chalk.grey(ports));
  ...
  connect: (portOrPath, callback) => {
    ...
    if (typeof portOrPath === "object" && portOrPath.path) {
      ...
      console.log(portOrPath.transport || "SerialPort", chalk.grey(path));
    } else {
      //
    ...
    try {
      io = new IO(portOrPath, error => {
        ...
        callback(this, caught, caught ? "error" : "ready", io);
      });
...
function Board(opts) {
  ...
  if (!opts.port && this.io !== null) {
    /* istanbul ignore next */
    console.log("Available", chalk.grey(this.io.name || "unknown"));
...
function finalizeAndBroadcast(data, type, io) {
  ...
  if (type === "connect") {
    ...
    console.log("Connected", chalk.grey(this.port));
...
Board.prototype.log = function(/* type, klass, message [, long description] */) {
  ...
  const klass = args.shift();
  let message = args.shift();
  const color = logging.colors[type];
  const now = Date.now();
  const event = {
    type: type,
    timestamp: now,
...
Board.mount = function(arg) {
  let index = typeof arg === "number" && arg,
    hardware;
...
Board.Component = function(opts, componentOpts) {
  ...
  if (typeof opts.pin === "number" || typeof opts.pin === "string") {
    ...
    if (Array.isArray(opts.pins)) {
      ...
      if (typeof opts.pins === "object" && opts.pins !== null) {
        const pinset = opts.pins || opts.pin;

        originalPins = [];
        for (const p in pinset) {
  ...
  //     const hasController = typeof request.controller !== UNDEFINED;
  //     const hasAddress = typeof request.address !== UNDEFINED;
  ...
  //         const isPinOccupied = request.value === occupied.value && request.type === occupied.type;
...
Board.Controller = function(controller, options) {
  const requirements = controller.requirements && controller.requirements.value;
...
function Boards(opts) {
  ...
  const initialized = {};
  const noRepl = ports.some(function(port) {
    return port.repl === false;
  });
  const noDebug = ports.some(function(port) {
  ...
  const expecteds = ports.map(function(port, index) {
    let portOpts;
  ...
  Promise.all(expecteds).then(
    function(boards) {
      ...
      this.each(function(board) {
        console.log("Board ID: ", chalk.green(board.id));
      });
