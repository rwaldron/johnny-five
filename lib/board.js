...
Serial = {
  ...
  connect: function( usb, callback ) {
    var err, firmata, isConnected, type;
    ...
    try {
      firmata = new Firmata( usb, function( error ) {
        if ( error !== undefined ) {
          err = error;
        }

        // Execute "ready" callback
        callback.call( this, err, "ready", firmata );
      }.bind(this));
    ...
    callback.call( this, err, type, firmata );
  }
};
...
function Board( opts ) {
  ...
  if ( this.mock ) {
    ...
    // If you already have a connect firmata instance
