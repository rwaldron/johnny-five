...
Board.prototype.serialize = function(filter) {
  ...
  return JSON.stringify(
    _.map(this.register, function(device) {
      // if there is only one device, return it; otherwise
      // return an array.
      if (device.length === 1) {
        return reduce.call(this, _.first(device));
      }
      return device.map(reduce, this);
    }, this)
  );
};
...
Board.Device = function(opts) {
  var register, addr, devicesAtAddr;

  // Board specific properties
  this.board = Board.mount(opts);
  register = this.board.register;
  addr = this.addr;
  devicesAtAddr = register[addr] || [];
  ...
  if (devicesAtAddr.length) {
    this.board.warn("Address %s already in use!", addr);
  }
  register[addr] = devicesAtAddr.concat(this);
};
