    ...
    scaled: {
      get: function() {
        ...
        if (state.scale && raw !== null) {
          ...
          mapped = Board.fmap(raw, this.range[0], this.range[1], state.scale[0], state.scale[1]);
...
Sensor.prototype.scaleTo = function(low, high) {
  ...
  return Board.map(this.raw, 0, 1023, scale[0], scale[1]);
};

Sensor.prototype.fscaleTo = function(low, high) {
  var scale = Array.isArray(low) ? low : [low, high];
  return Board.fmap(this.raw, 0, 1023, scale[0], scale[1]);
};
