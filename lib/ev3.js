...
EV3.shieldPort = function(pin) {
  ...
  if (pin.includes("S")) {
    ...
    analog = endsWithS1 ? EV3.S1_ANALOG : EV3.S2_ANALOG;
    // Sensor Mode (1 or 2?)
...
EV3.isRawSensor = function(port) {
  return port.analog === EV3.S1_ANALOG || port.analog === EV3.S2_ANALOG;
};
...
EV3.prototype.read = function(port, register, numBytes, callback) {
  ...
  if (port.sensor && port.offset && !EV3.isRawSensor(port)) {
