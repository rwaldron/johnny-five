...
Animation.prototype.next = function() {
  ...
  if (this.segments.length > 0) {
    ...
    // Device type specific normalization
    this.keyFrames = this.target[Animation.normalize](this.keyFrames);

    // Normalize the keyFrames to match expected structure
    this.keyFrames = normalizeKeyframes(this.keyFrames, this.cuePoints);
 ...
 * Start a segment
 *
 * @param  {Object} opts Options: reverse, loop, progress
 *
 */
...
Animation.prototype.play = function(opts) {
  ...
  this.playLoop = temporal.loop( this.rate, function(loop) {
    ...
    var progress,
      indices = { left: null, right: null },
      tween = { duration: null, progress: null };

    progress = (loop.calledAt - this.startTime) / this.scaledDuration;
    ...
    var val = this.keyFrames.map(function(keyFrame) {
      // Note: "this" is bound to the animation object

      var memberIndices = { left: null, right: null };
      ...
      if (right.position) {
        ...
        calcValue = (right.degrees - left.degrees) *
          tween.progress + left.degrees;
      }
...
var normalizeKeyframes = function(keyFrameSet, cuePoints) {
    ...
    keyFrames.forEach(function (keyFrame, i, source) {
      ...
      if (keyFrame !== null) {
        ...
        if (typeof keyFrame !== "object") {
          keyFrame = { step: keyFrame, easing: "linear" };
        }

        // Replace step values
        if (typeof keyFrame.step !== "undefined") {
          keyFrame.degrees = keyFrame.step === false ?
            previousDeg : previousDeg + keyFrame.step;
        }
        ...
        // Copy degrees from another frame
        if (typeof keyFrame.copyDegrees !== "undefined") {
          keyFrame.degrees = source[keyFrame.copyDegrees].degrees;
        }
        ...
        previousDeg = keyFrame.degrees;

      } else {

        if (i === source.length - 1) {
          keyFrame = { degrees: previousDeg, easing: "linear" };
        } else {
          keyFrame = null;
        }
