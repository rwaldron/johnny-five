...
function Servo(opts) {
  ...
  // This is temporary and will change with issue #246
  this.resolution = 255;
...
Servo.prototype.to = function(degrees, time, rate) {
  ...
  if (typeof time !== "undefined") {
    ...
    rate = rate || (time / 1000) * this.fps;

    last = this.last && this.last.degrees || 0;
    distance = Math.abs(last - degrees);
    percent = 0;

    // If steps are limited by pwm resolution use calculated value
    var maxSteps = distance / ( 180 / this.resolution );
    if (maxSteps < rate) {
      rate = maxSteps;
    }
